...
export default {
  ...
  methods: {
    ...
    renderNodeLink(){
      ...
      let topNodes = Object.fromEntries(Object.entries(this.corrCluster)
          .map(([k, v]) => [
            k,
      ...
      // let x = Object.fromEntries(Object.entries(topNodes)
      //     // eslint-disable-next-line no-unused-vars
      //     .map(([k, v]) => [
      //         k,
      //         this.corrCluster[k].components.filter()
      //
      //         ]
      //     )
      // )
      ...
      let graphX = d3.scaleLinear()
          .domain([0, 10])
          .range([this.distPaddingWidth*2, this.width-this.distWidth-this.distPaddingWidth]);
      let graphY = d3.scalePoint()
          .domain(Object.keys(this.corrCluster).sort().reverse())
          .range([
            this.distHeight*0.75,
            this.distHeight*0.75+(this.distHeight+this.distPaddingHeight)*9
          ]);
      ...
      this.svg
          ...
          .append('g')
          .attr('class', 'graph')
          .attr('transform', d => `translate(${this.distWidth}, ${graphY(d[0])})`)
          .each( (d, i, c) => {
